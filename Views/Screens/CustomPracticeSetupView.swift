            ...
            ForEach(0..<2) { index in
                Button(action: { withAnimation { selectedTab = index } }) {
                    Text(index == 0 ? "HSK Levels" : "Categories")
                        ...
                        .foregroundColor(selectedTab == index ? .white : .secondaryText)
                        .frame(maxWidth: .infinity)
                        .padding(.vertical, 12)
                        .background(
                            RoundedRectangle(cornerRadius: 12)
                                .fill(selectedTab == index ? Color.midnightGreen : Color.clear)
                        )
                }
            }
        }
...
private struct CategorySelectionCard: View {
    ...
    var body: some View {
        Button(action: action) {
            VStack(spacing: 12) {
                ZStack {
                    ...
                    Image(systemName: category.icon)
                        .font(.title2)
                        .foregroundColor(isSelected ? .white : .midnightGreen)
                }
            ...
            .background(
                RoundedRectangle(cornerRadius: 16)
                    .stroke(isSelected ? Color.midnightGreen : Color.lightSilver.opacity(0.3), lineWidth: 1)
            )
        }
        .buttonStyle(PlainButtonStyle())
    }
